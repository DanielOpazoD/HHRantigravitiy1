...
export const CensusActionsProvider: React.FC<CensusActionsProviderProps> = ({ children }) => {
    ...
    const executeDischarge = useCallback(() => {
        if (dischargeState.recordId) {
            updateDischarge(dischargeState.recordId, dischargeState.status);
        } else if (dischargeState.bedId) {
            addDischarge(dischargeState.bedId, dischargeState.status, dischargeState.clinicalCribStatus);
        }
